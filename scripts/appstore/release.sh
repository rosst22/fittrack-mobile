...
# Passing CURRENT_PROJECT_VERSION to xcodebuild is NOT enough on its own.
# expo prebuild writes a literal <string>1</string> for CFBundleVersion rather
# than $(CURRENT_PROJECT_VERSION), so the override is silently ignored and every
# build uploads as "1" — which Apple accepts exactly once, then rejects every
# later upload for not incrementing. Write it into the plist directly.
#
# ios/ is gitignored and regenerated by prebuild, so this has to happen here on
# every run rather than being a one-time edit to the generated file.
PLIST="ios/FitTrackAI/Info.plist"
/usr/libexec/PlistBuddy -c "Set :CFBundleVersion $BUILD_NUMBER" "$PLIST"
echo "▸ Set CFBundleVersion in $PLIST to $(/usr/libexec/PlistBuddy -c 'Print :CFBundleVersion' "$PLIST")"
