...
export default function TrendsScreen() {
  const { data, error, refreshing, onRefresh } = useAsync(async () => {
    ...
    return { days, meals, workouts, goals };
  }, []);

  let content = null;
  if (data) {
    const { days, meals, workouts, goals } = data;
    ...
    const byDay = new Map(days.map((d) => [d, { eaten: null as number | null, protein: null as number | null, carbs: null as number | null, fat: null as number | null, burned: 0 }]));
    ...
    for (const w of workouts) {
      ...
      if (slot) slot.burned += sumCaloriesBurned([w]);
    }
